...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    ...
    var pContain= document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(pContain[0], size);
    var newwidth = (pContain[0].offsetWidth + dx) + 'px';
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    phase = Math.sin(currentScrollY + (i % 5)) * 100 + items[i].basicLeft ;
